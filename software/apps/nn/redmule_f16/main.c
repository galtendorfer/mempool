...
int main() {
  ...
  // Compute
  redmule_synch_single(l1_X, l1_Y, l1_W, matrix_M, matrix_N, matrix_P, GEMM);
  ...
  if (core_id == 0) {
    for (uint32_t i = 0; i < num_redmules; i++) {
      uint32_t X_shift = (SHIFT == 1) ? (i * PORT_WIDTH) % matrix_N : 0;
  ...
  // Compute
  redmule_synch_parallel(l1_X, l1_Y, l1_W,
